...
resource "aws_ecr_lifecycle_policy" "app" {
  ...
  policy = jsonencode({
    rules = [
      {
        rulePriority = 1
        description  = "Ponechat posledních 10 image"
        selection = {
          tagStatus   = "any"
...
# ==============================================================
# IAM Role pro ECS Task Execution
# ==============================================================
...
# Přístup k SSM Parameter Store (pro secrets)
...
# IAM Role pro samotný Task (přístup k AWS zdrojům z aplikace)
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        for k, v in var.app_environment_vars : { name = k, value = v }
      ]
...
resource "aws_ecs_service" "app" {
  ...
  lifecycle {
    ignore_changes = [desired_count] # Řídí auto-scaler
  }
}
